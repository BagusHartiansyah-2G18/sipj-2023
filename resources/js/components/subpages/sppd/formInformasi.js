import React from "react";
import sfLib from "../../mfc/sfLib";
import PropTypes from "prop-types";

function FormInformasi({dt}) {
    return (
        <div class="FM1 ">
            <div class="header">
                <div class="cdark flexR">
                    <button className="btn bnone">
                        <span className="mdi mdi-star-crescent cwarning fzXl"></span>
                    </button>
                    <h2 className="  pl0 aiE fBebasNeue">
                        <b>Informasi</b> 
                    </h2>
                </div> 
            </div>
            <div class="body bdark pm0 bsolid1 " style={{width:"unset"}}><br/>
                <div className="flexC">
                    <div>
                        <ul style={{listStyle:"none"}}>
                            <li className="ptb10px">
                                <label className="tbold">Sub Kegiatan :</label><br></br>
                                <span className="fzXl">{dt.nmSub}</span>
                            </li>
                            <li className="ptb10px">
                                <label className="tbold">Uraian Belanja :</label><br></br>
                                <span className="fzXl">{dt.nama}</span>
                            </li> 
                            {/* <li className="ptb10px">
                                <label className="fbold">Pagu Triwulan</label><br></br>
                                <div className="flexR justifySE">
                                    <div>
                                        <label className="fbold">TW 1</label><br></br>
                                        <span>Rp.<small>{sfLib._$(dt.tw1)}</small></span>
                                    </div>
                                    <div>
                                        <label className="fbold">TW 2</label><br></br>
                                        <span>Rp.<small>{sfLib._$(dt.tw2)}</small></span>
                                    </div>
                                    <div>
                                        <label className="fbold">TW 3</label><br></br>
                                        <span>Rp.<small>{sfLib._$(dt.tw3)}</small></span>
                                    </div>
                                    <div>
                                        <label className="fbold">TW 3</label><br></br>
                                        <span>Rp.<small>{sfLib._$(dt.tw4)}</small></span>
                                    </div>
                                </div>
                            </li> */}
                        </ul>
                    </div>  
                    <div className="grid-col3" style={{gap:"0px"}}>
                        <div className="flexR bprimary aiC">
                            <span className="mdi mdi-cart cwhite  fzL2 "></span>
                            <h3 className="pwrap_10">
                                <label className="fbold">Sisa</label><br></br>
                                <span>Rp.<small>{sfLib._$(dt.total-dt.realisasi)}</small></span>
                            </h3>
                        </div>

                        <div className="flexR bsuccess aiC">
                            <span className="mdi mdi-currency-usd cwhite rotasiM45 fzL2 "></span>

                            <h3 className="pwrap_10">
                                <label className="fbold">Alokasi</label><br></br>
                                <span>Rp.<small>{sfLib._$(dt.total)}</small></span>
                            </h3>
                        </div>
                        <div className="flexR bdanger aiC">
                            <span className="mdi mdi-currency-usd-off cwhite rotasiM45 fzL2 "></span>

                            <h3 className="pwrap_10">
                                <label className="fbold">Realisasi</label><br></br>
                                <span>Rp.<small>{sfLib._$(dt.realisasi)}</small></span>
                            </h3>
                        </div>
                    </div>
                </div>
            </div>
        </div> 
    );
}
FormInformasi.propTypes = {
    dt : PropTypes.object.isRequired,
}
export default FormInformasi;
